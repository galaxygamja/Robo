/*
  Hamster Robotics Challenge Robot v0.9
  Purchased-hardware revision. The v0.8 capture and stability geometry is
  preserved while the N20 motor, D-shaft wheel and SCX24-class battery bays
  are resized to the procurement-sheet hardware.
...
// The v0.8 gate_slider remains ONE rigid part.
...
// ---------------------------
robot_width = 100;
// The 56 mm battery requires a short rear service extension. The protected
// capture geometry remains at its original coordinates.
robot_length = 112;
robot_height = 100;
...
front_gate_bottom_clearance = 0.8;
...
// Procurement v2.8 item 7: HJ-2S, 7.4 V, 350 mAh, 30 C. The spreadsheet does
// not state a package size; this reference uses the published SCX24-compatible
// OEM envelope 56 x 28 x 13 mm. Measure the delivered HJ pack before printing.
battery_length = 56;
battery_width = 28;
battery_height = 13;
...
// Procurement v2.8 item 1 / Eduino D-75, option 6 V 100 RPM.
// Seller drawing: overall 33 x 12 x 10 mm, shaft projection 9 mm.
// The matching N20 D-shaft wheel uses the standard 3 mm D shaft.
motor_length = 33;
motor_width = 12;
motor_height = 10;
...
// Procurement v2.8 item 2 / Eduino D-105 seller drawing.
wheel_diameter = 43;
wheel_width = 17.5;
wheel_clearance = 2.5;
wheel_z_offset = 0;
...
wheel_axial_gap = 1.0;
// Move each N20 outward to leave a clear 30 mm central battery corridor.
motor_inner_target_x = battery_width / 2 + motor_clearance
                     + motor_mount_wall + 0.6;
motor_center_x = motor_inner_target_x + motor_length / 2;
motor_inner_x = motor_center_x - motor_length / 2;
motor_outer_x = motor_center_x + motor_length / 2;
wheel_center_x = motor_outer_x + wheel_axial_gap + wheel_width / 2;
...
caster_diameter = 12;
caster_mount_width = 46;
caster_mount_length = 14;
caster_mount_hole_spacing = 38;
caster_height = 12;
caster_center_y = -40;
...
// v0.8: controlled front contact points. The v0.7 printed-structure centroid
// was approximately Y=-1.6 mm, ahead of the Y=-15 mm drive axle. These small
// skids extend the support polygon to Y=+39 mm without entering the disk path.
// Apply PTFE/UHMW tape to the contact faces after printing for lower scrub.
front_skid_center_x = 47.25;
front_skid_center_y = 34;
front_skid_length = 10;
front_skid_width = 5;
front_skid_contact_z = 0;
front_skid_height = 1.5;
review_structural_cg_y = -1.6;
front_skid_to_offset_disk_margin =
    front_skid_center_x-front_skid_width/2
    -(funnel_capture_offset_target+sample_radius);

// Low central battery tray placement and service hardware.
...
battery_rear_bolt_y = -50;
battery_stop_wall = 2;
battery_front_hanger_x = 19;
...
motor_shaft_tip_x = motor_outer_x + motor_shaft_length;
motor_shaft_insertion = motor_shaft_tip_x - wheel_inner_face_x;
hardware_overall_width = 2 * (wheel_center_x + wheel_width / 2);
...
assert(front_gate_down_bottom_z >= 0.8,
       "Moving gate is too close to a realistic uneven arena floor");
assert(guide_clearance >= 0.5,
       "Printed gate guide needs at least 0.5 mm clearance per side");
assert(front_skid_center_x + front_skid_width/2 <= robot_width/2,
       "Front skid exceeds the chassis width");
assert(front_skid_to_offset_disk_margin >= 1.5,
       "Front skid enters the nominal off-centre disk path");
assert(review_structural_cg_y > motor_center_y
       && review_structural_cg_y < front_skid_center_y,
       "Reviewed structural CG is outside the wheel/front-skid support span");
...
assert(motor_length == 33 && motor_width == 12 && motor_height == 10,
       "Motor reference must match the procurement-sheet D-75 envelope");
assert(motor_shaft_diameter == 3 && motor_shaft_length == 9,
       "Motor shaft reference must remain D3 x 9 mm");
assert(wheel_diameter == 43 && wheel_width == 17.5,
       "Wheel reference must match procurement-sheet item 2");
assert(motor_shaft_insertion >= 5,
       "D shaft does not engage deeply enough with the wheel hub");
assert(battery_length == 56 && battery_width == 28 && battery_height == 13,
       "Battery reference must remain at the SCX24-compatible envelope");
...
echo(str("v0.9 state: ", front_gate_state));
...
echo(str("V0.9 hardware: motor = ",[motor_length,motor_width,motor_height],
         ", D",motor_shaft_diameter," x ",motor_shaft_length,
         "; wheel = D",wheel_diameter," x ",wheel_width,
         "; shaft insertion = ",motor_shaft_insertion," mm"));
echo(str("V0.9 battery = ",[battery_length,battery_width,battery_height],
         "; overall robot envelope = ",[hardware_overall_width,robot_length,robot_height]));
...
echo(str("V0.9 support Y span = ", motor_center_y, " to ",
         front_skid_center_y, " mm; reviewed structure CG Y = ",
         review_structural_cg_y, " mm"));
echo(str("V0.9 skid to +/-",funnel_capture_offset_target,
         " mm offset disk margin = ",front_skid_to_offset_disk_margin," mm"));
...
module front_stability_skids() {
    for (s = [-1, 1])
        translate([s * front_skid_center_x, front_skid_center_y,
                   front_skid_contact_z])
            linear_extrude(height = front_skid_height)
                capsule_2d([0, -(front_skid_length-front_skid_width)/2],
                           [0,  (front_skid_length-front_skid_width)/2],
                           front_skid_width);
}
...
// Part selector
// ---------------------------
// v0.8 manufacturing joints. All coordinates remain in the assembly frame.
...
module rear_caster_mount() {
    difference() {
        union() {
            ...
            // Split rear flange leaves the 28 mm battery extraction corridor open.
            for(x=[-caster_mount_hole_spacing/2,caster_mount_hole_spacing/2])
                translate([x-4.5,caster_back_y,caster_height])
                    cube([9,3,caster_flange_height]);
            for(x=[-caster_mount_hole_spacing/2,caster_mount_hole_spacing/2])
...
if (part_to_render == "assembly") {
    ...
    // v0.8 skids define Z=0; no legacy -side_rail_bottom_z shift is needed.
    lower_chassis_with_upper_mounts();
