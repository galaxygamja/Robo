...
> 재시작 후속: [영속 장부와 장부 연결 송신기](OPERATION_JOURNAL_KO.md)를 추가했다.
> 아래 기본 ManipulatorSender의 메모리 이력 제한은 그대로이며, 장부를 명시적으로 사용하는
> 별도 wrapper에만 재시작 차단이 적용된다. 실제 물리 exactly-once 보장은 아니다.
