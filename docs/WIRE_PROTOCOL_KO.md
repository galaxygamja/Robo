...
> 2026-09-08 조작 후속: [별도 조작·센서 규격 초안과 시험](MANIPULATOR_PROTOCOL_KO.md)을 추가했다.
> 아래 주행 v1은 변경하지 않았고, 새 합성 센서를 실제 runtime feedback으로 변환하지 않는다.

> 후속: [runtime 감시·전체 통신 오류·보고서 통합](RUNTIME_WIRE_KO.md)을 구현했다.
> `--mission ... --wire-fake`가 명시된 경우에만 연결하며, 실제 매체·장치 송신은 여전히 미구현이다.
...
받는 API이며 후속 `--wire-fake` 모드는 실제 runtime 감시에 가짜 송수신을 연결한다.
실제 장치 송수신을 켜는 옵션은 없다. 기존 `adapters.UdpRobotTransport`는 다른
...
- 송신 API 검증 실패/ACK 거부/시간 초과의 전체 보류·최선의 정지 요청·명시적 재연결을
  후속 `RuntimeWireSupervisor`와 `--wire-fake` 모드에 구현했다. 실제 네트워크/장치
  어댑터의 장애 처리와 펌웨어 검증까지 완료했다는 뜻은 아니다.
