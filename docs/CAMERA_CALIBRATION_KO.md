...
> 2026-09-07 확장: [렌즈 보정/사진 수집](LENS_CALIBRATION_KO.md)을 구현했다.
> `calibrate --lens` 사용 시 schema 2에 렌즈 값을 내장한다. 아래 렌즈 없는 schema 1도 유지한다.
...
렌즈 왜곡 보정은 2026-09-07에 선택 기능으로 추가했다. 바닥보다 높은 로봇 태그의 시차 보정과
자동 모서리 검출은 아직 미구현이다. 광각은 Brown5 모델이 충분한지도 독립 기준점으로 확인하며,
fisheye는 현재 모델로 억지로 맞추지 않는다.
