...
기본 `detect` 결과는 프레임별 **관측값**이다. `--track`은 위치 이력·속도·누락/만료를 추가한다. 가림 구간 예측과 ESP32 명령 송신·모터 제어는 아직 연결하지 않았다.
...
누락된 로봇은 `missing_robot_ids`에 들어간다. 새 `--track`은 해당 프레임을 즉시 제어 불가로 표시하고 마지막
정상 관측 시각부터 500ms 후 `stale`로 표시한다. 실제 정지 송신은 미구현이며 제어기·펌웨어 watchdog과 연결해야 한다.
