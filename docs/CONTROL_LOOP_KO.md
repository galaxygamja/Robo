...
2026-09-07 추가: `field_size_mm=(폭, 높이)`를 지정하면 목표의 차체 반경+여유가
네 외벽 안에 들어오는지 생성 시 검사한다. tick에서는 측정/제안 속도의 swept 영역과
반응·제동 여유도 외벽 안에 있어야 한다. 위반은 `field_boundary_envelope` conflict로 기록한다.
새 runtime은 보정된 필드 크기를 반드시 전달한다. 넓은 합성 `--demo`는 이 옵션을 쓰지 않으므로
해당 데모의 수렴 결과를 경기장 경계 시험으로 해석하지 않는다.
