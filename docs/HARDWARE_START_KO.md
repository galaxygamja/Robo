...
> **이 문서는 이전 DC 모터 공통 설계 기록입니다. 최신 햄스터의 배선에 그대로 사용하지 마세요.** 2026-09-21부터 햄스터는 SM-S4303R 연속회전 서보2개로 구동하고 비버만 N20+DRV8833를 사용합니다. 새 스케치·배선·업로드 절차는 [Arduino 시작 안내](ARDUINO_START_HERE_KO.md)를 따르세요. Python 서버와 schema1은 변경하지 않았지만, 이전 DC 구동용 속도 보정값은 새 햄스터 구동 방식에 재사용하지 말고 다시 실측해야 합니다.
