...
> 후속 구현: [`--mission` 실행 모드](MISSION_ROUTE_RUNTIME_KO.md)는 WorldState → 임무 →
> A* → 차동 차체 속도를 연결한다. 아래 메카넘/단일 목표 설명은 기존 `--goals` 모드다.

> 장치 합의 갱신: [2륜 차동구동](DEVICE_COMMAND_SPEC_KO.md)이 확정됐다. 아래 runtime의
> 목표 속도/4륜 메카넘 mock 출력은 아직 차동용으로 수정 전이며, 실물 명령으로 사용할 수 없다.
...
기존 `--goals` 모드는 명시 지점의 목표 속도를 계산할 뿐 A* 경로, 의료 물체 집기/해제, 임무 순서,
