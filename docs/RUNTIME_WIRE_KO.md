...
같은 날 운영 후속에서 ACK 대기 중 로컬 출력 0과 실제 전송 대기 명령을 구분하도록 보강했다.
현재 측정한 로봇 자세와 장애물에 대해 수신기 모델의 기존 목표 및 미확인 명령을 모두
검사한 뒤 큐를 전달한다. [관측 대상·장애물·영상 통합 검증](HARDWARE_FREE_OPERATIONS_KO.md)을 참고한다.
