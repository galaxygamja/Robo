...
> 운영 준비 후속: [대상 연결·관측 기반 픽업점](HARDWARE_FREE_OPERATIONS_KO.md).
> schema 1은 기존 고정 접근점이며, schema 2는 명시적 대상 연결과 검토한 접촉점 오프셋으로
> 픽업 목표를 새 관측에서 계산한다. 아래 초기 버전의 동적 픽업점 미구현 표기는 이 범위에서 대체한다.
