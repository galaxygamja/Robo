/*
  Hamster Robotics Challenge Robot v0.8
  Physical-stability revision. The v0.7 capture arcs are preserved;
  front anti-tip skids, safer floor clearance and production slide clearance
  are added without replacing the serviceable v0.7 joints.
...
// Purchased drive target (x2): 6 V brushed DC metal gearmotor, 100..200 rpm
// loaded output speed, >=0.8 kgf.cm continuous torque, and <=1.5 A stall
// current per motor. The COMPLETE body/gearbox/terminal envelope must fit the
// dimensions below. The output is a D shaft, nominally D3 x <=12 mm. This is a
// fit specification, not a declaration that every "N20" listing is identical.
motor_length = 30;
motor_width = 15;
motor_height = 20;
...
// Purchased battery target: protected 2S LiPo, 7.4 V nominal / 8.4 V full,
// 300..500 mAh, >=20 C. The wrapped pack, protection/balance lead and cable
// bend must all fit the envelope below. Use a >=4 A 6 V regulator for the drive
// rail and a separate >=3 A 5 V BEC for controller/servo power; never feed a
// micro servo directly from a fully charged 2S pack.
battery_width = 24;
battery_length = 36;
battery_height = 20;
battery_clearance = 2;
...
echo(str("v0.8 state: ", front_gate_state));
...
echo(str("V0.8 support Y span = ", motor_center_y, " to ",
         front_skid_center_y, " mm; reviewed structure CG Y = ",
         review_structural_cg_y, " mm"));
echo(str("V0.8 skid to +/-",funnel_capture_offset_target,
