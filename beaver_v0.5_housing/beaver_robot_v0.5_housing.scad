/*
  Beaver Robot v0.5-housing - removable rounded upper protective shell
  ...
  retained. This revision adds one separately printable 2.0 mm rounded shell
  around the central servo/electronics space. Front, cargo-top and rear-dump
  windows remain open. Four horizontal M3 screws make the shell removable.
*/

$fn = 64;
part = "housing_complete_close";
// Select: housing_complete_close, housing_gate_open, housing_only,
...
cargo_slope_rise = 2.5;
cargo_slope_angle = atan(cargo_slope_rise/cargo_inner_length);
cargo_side_wall_top_z = 83.5;
cargo_front_wall_top_z = 90.5;
...
upper_box_center_y = cargo_box_center_y - box_size[2]*sin(cargo_slope_angle);
payload_two_box_center_y = (cargo_box_center_y + upper_box_center_y)/2;
...
assert(wheel_arch_clearance >= 1.5,
       "Wheel arch needs at least 1.5 mm radial running clearance");
...
module integrated_chassis_assembly(print_z0=false) {
    translate([0,0,print_z0 ? -chassis_ground_z : 0])
...
module upper_housing_assembly(print_z0=false) {
    translate([0,0,print_z0 ? -housing_bottom_z : 0])
        upper_housing_raw();
}
...
// ---------------- Part selector ----------------
if (part == "housing_complete_close") {
