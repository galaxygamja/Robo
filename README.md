...
> 최신 변경: **재료 없는 모의 폐루프 제어·색 물체 연속 추적·혼잡 회복·안전 시험실**.
> [최신 실행법·변경 내용·검증 결과](docs/SOFTWARE_REHEARSAL_KO.md)를 먼저 보세요. Python 242 / 웹 35 시험 통과.
...
현재 실제 비전 CLI는 실행당 카메라 한 대를 처리한다. 카메라 수를 2대로 고정하지 않습니다. 다중 영상 시간 동기화·관측 융합은 아직 구현하지 않았습니다. 카메라별 보정과 단일 영상 검출을 먼저 실측한 뒤 연결한다.
...
현재 기본 팀은 지상로봇 4대이며 드론 없이 좌표 관측을 모의합니다.
...
2026-09-05 현재 vision 의존성 환경에서 Python 183개·웹 30개 회귀시험을 통과했습니다.
