...
> schema 2 관측 픽업점과 `--object-obstacles`의 동적 장애물 지도도 선택할 수 있습니다.
> 물체 크기/집게 오프셋은 검토한 입력만 사용하며, 센서 없는 집기 성공이나 접촉 예외를 만들지 않습니다.
> `tools/verify_mission_operations.py`로 생성 영상 통합 3개 시나리오와 선택 관측 전용 지속 시험을 재현합니다.
...
> 장치 합의(2026-09-07): **지상 4대는 2륜 차동구동, 옆 이동 불가**로 확정했습니다.
> 엔코더는 미장착 예상이며 아직 미확정입니다. [장치·명령 규격](docs/DEVICE_COMMAND_SPEC_KO.md)을 우선하세요.
> 실제 운용용 `--mission`은 차동 차체 명령을 계산합니다. 기존 `--goals`/웹 검증 모드의 메카넘 가정과
> 구분해야 하며, 어느 모드도 현재 실물 모터 출력으로 연결되어 있지는 않습니다.

> 2026-09-07 후속: [렌즈 왜곡 보정·원본 사진 수집](docs/LENS_CALIBRATION_KO.md)을 추가했습니다.
> 기존 카메라/태그/runtime에 연결되며 실제 카메라 정확도·모터 출력은 아직 실물 검증하지 않았습니다.
> [이번 후속 작업 기록](docs/LENS_WORK_20260907_KO.md)도 확인하세요.

> 최신 변경(2026-09-07): **실제 고정 카메라 → 기존 추적 → 목표 속도 계산 런타임**.
> [카메라 실행법과 안전 경계](docs/REAL_CAMERA_RUNTIME_KO.md), [작업·검증 기록](docs/REAL_RUNTIME_WORK_KO.md)을 먼저 보세요.
> 영상 읽기/검출은 별도 프로세스, 관측 만료는 중앙 루프에서 감시합니다. **실제 모터 출력은 미연결**입니다.
> 기존 태그 0=H1 / 1=H2 / 2=B1 / 3=B2는 유지하며 **H2가 세 번째 비버(B3)** 입니다.

> 팀 작업을 시작하기 전에 [전체 프로젝트 기록](ROBOTICS_PROJECT_FULL_RECORD_KO.txt)의
> `2026-09-07 팀 작업 인계`와 [소프트웨어 중복 방지 인계표](docs/SOFTWARE_PLAN_KO.md)를
> 확인하세요. 보정·태그·연속 추적·HSV 물체 검출/추적은 이미 구현됐습니다.

웹 동선 최적화의 가상 45.18초/160점(최신 1·2·1 적재), 이전 47.06초는 실물 성능이 아닙니다.
[팀원의 동선 비교](web_simulator/ROUTE_OPTIMIZATION_KO.md)와
[이전 드론 관측 비교](docs/COORDINATION_COMPARISON_FIX_KO.md)는 시뮬레이션 자료입니다.

### 실제 카메라 연속 실행 (출력은 기록 전용)

`camera.json`은 현장에서 기존 보정 명령으로 먼저 생성합니다. 목표를 생략하므로
모든 속도 출력은 0입니다. 보고서는 매번 새 파일 이름을 사용합니다.

```powershell
.\.venv\Scripts\python.exe -m robo_control.runtime --camera 0 --calibration recordings/camera.json --fleet config/qualifier_senior.json --report recordings/runtime-001.jsonl --duration-s 120
.\.venv\Scripts\python.exe -m robo_control.runtime_report recordings/runtime-001.jsonl
```

명령으로 가상 위치를 전진시키지 않습니다. 200ms 관측 만료, 300ms 검증용 명령 TTL,
연속 3프레임 재확인, 오류 시 세션 종료를 적용합니다. 펌웨어 watchdog과 물리 비상정지는 별도 필요합니다.
...
처리 로그를 제공합니다. `detect --track`으로 연속 관측·속도·누락/만료 검사를 지원합니다. ESP32 주행 연동은 아직 미구현입니다.
...
현재 실제 비전 CLI는 실행당 카메라 한 대를 처리한다. 카메라 수를 2대로 고정하지 않습니다. 새 `active_observer`는 여러 입력원 중 로봇별 좌표 하나를 고르는 독립 계약/모의 모듈입니다. 실제 다중 영상 시간 동기화·동적 보정·추적기 연결과 Kalman/EKF 융합은 아직 구현하지 않았습니다.

## 현재 기본 구성: 햄스터 1 · 비버 3 · 실시간 좌표 추적
...
현재 기본 팀은 지상로봇 4대이며 웹 첫 화면은 박쥐 1대의 이동 관측 모드입니다.
드론 없이 좌표 관측을 쓰는 선택도 유지합니다. 더 이상 지상로봇 6대가 아닙니다.

- 햄스터 1대: 디스크 3개를 서로 다른 LAB 원으로. 게이트·광센서 모의.
- 비버 3대: 빨강 3개→H, 초록 3개→RZ, 노랑 3개→양쪽 PCC. 집게 서보 모의.
- 의료키트 큐브: B1 1개→PCC-L, B2 2개→H, H2(B3) 1개→PCC-R 전담. 배출 서보 모의.
- 실시간 좌표: 10Hz 이상적 관측 모의, x/y mm·방향·누락·나이 표시. 실제 JSONL도 로컬에서 확인.
- 박쥐: 가림·누락·작업 필요도에 따라 위치와 높이를 선택하고 기본 위치 입력을 보완. 드론 없음/중앙 정지/이동 관측 비교.
...
최신 1·2·1 적재 웹 모델의 기본 69.66초/선정 동선 45.18초는 가상 주행 시간이며 Python의 단순 합성 시간과 성능 비교하면 안 됩니다. 정상 정보가 같으면 드론 유무로 시간을 인위적으로 바꾸지 않습니다. 이전 적재의 58초 및 간헐 누락 비교는 과거 실험 기록입니다.
웹·Python은 같은 좌하단 원점/물체/목적지/작업 장면을 쓰지만 서로 실시간 연결된 것은 아닙니다.
`python -m robo_control.control_loop --demo --compact`로 측정 좌표 기반 모의 폐루프를 실행할 수 있습니다. 모터·드론·무선통신과 공개 웹의 실시간 연결은 재료 확보 뒤 진행합니다.
...
새 Python 관측 데모: `python -m robo_control.active_observer --demo --compact`. 실제 기체 명령을 내보내지 않습니다.
