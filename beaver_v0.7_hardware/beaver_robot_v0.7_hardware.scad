/*
  Beaver Robot v0.7-hardware - N20 and SCX24 battery integration
  Units: millimetres. +Y is forward, Z=0 is the arena floor.

  The v0.6 front gripper, linkage, cargo dumper and removable housing are
  retained. This revision adds dimensioned N20 motor cradles, 43 mm D-shaft
  wheels and a low removable SCX24-format 2S battery tray. The front jaw arcs,
  pivots and linkage geometry are unchanged.
*/
...
// integrated_chassis, battery_tray, motor_mount_left, motor_mount_right,
// wheel_reference, motor_reference, battery_reference,
...
// ---------------- v0.7 drive hardware and compact body ----------------
// Purchase-sheet hardware: N20 6 V 100 RPM motor, 43 mm D-shaft wheel and
// HJ-2S 7.4 V 350 mAh 30 C SCX24-compatible battery.  The spreadsheet does
// not contain mechanical drawings; the parameter values below therefore use
// the seller motor/wheel drawings and the official SCX24 battery envelope
// documented in README.md. Measure the delivered battery before final print.
...
motor_clearance = 0.6;
motor_mount_wall = 2.4;
motor_center_x = 31.5;
motor_center_y = -23;
motor_center_z = 21.5;
motor_mount_inner_x = 29.6;
motor_mount_outer_x = motor_center_x+motor_length/2+motor_clearance;

wheel_diameter = 43;
wheel_width = 17.5;
wheel_axial_gap = 1;
wheel_center_x = motor_center_x+motor_length/2+wheel_axial_gap+wheel_width/2;
wheel_center_y = -23;
wheel_center_z = motor_center_z;
...
battery_center_y = -25;
battery_tray_base_z = chassis_ground_z;
battery_tray_thickness = 2;
battery_bottom_z = battery_tray_base_z+battery_tray_thickness;
battery_top_z = battery_bottom_z+battery_height;
battery_mount_x = 33;
battery_mount_y = [-34,-6];
battery_mount_hole_d = 3.4;
battery_mount_pad_d = 9;
battery_tray_tab_bottom_z = chassis_ground_z+3;
battery_tray_tab_thickness = 2;
motor_mount_foot_bottom_z = battery_tray_tab_bottom_z+battery_tray_tab_thickness;
motor_mount_foot_thickness = 2.4;

wheel_inner_face_x = wheel_center_x-wheel_width/2;
wheel_outer_face_x = wheel_center_x+wheel_width/2;
motor_outer_face_x = motor_center_x+motor_length/2;
motor_shaft_tip_x = motor_outer_face_x+motor_shaft_length;
motor_shaft_insertion = motor_shaft_tip_x-wheel_inner_face_x;
hardware_overall_width = 2*wheel_outer_face_x;
...
cargo_shoulder_rib_bottom_z = cargo_shoulder_bottom_z-9.5;
...
robot_min_x = -wheel_outer_face_x;
robot_max_x = wheel_outer_face_x;
robot_min_y = base_rear_y;
robot_max_y = max(open_outer_tip_y, open_rounded_tip_max_y);
robot_min_z = 0;
robot_max_z = cargo_front_wall_top_z;
...
assert(motor_length == 33 && motor_width == 12 && motor_height == 10,
       "N20 motor body parameters do not match the sourced 33 x 12 x 10 mm drawing");
assert(motor_shaft_diameter == 3 && motor_shaft_length == 9,
       "N20 D-shaft parameters do not match the sourced D3 x 9 mm geometry");
assert(wheel_diameter == 43 && wheel_width == 17.5,
       "Purchased wheel parameters must remain 43 x 17.5 mm");
assert(battery_length == 56 && battery_width == 28 && battery_height == 13,
       "SCX24-compatible battery envelope must remain 56 x 28 x 13 mm");
assert(motor_shaft_insertion >= 6 && motor_shaft_insertion <= motor_shaft_length,
       "Wheel hub does not provide adequate N20 shaft engagement");
assert(battery_top_z <= motor_center_z-motor_height/2,
       "Battery envelope enters the drive-motor bodies");
assert(motor_mount_inner_x-battery_length/2 >= 1,
       "Motor cradles enter the removable battery envelope");
assert(cargo_shoulder_rib_bottom_z-(motor_center_z+motor_height/2) >= 0.4,
       "Cargo shoulder rib needs printed clearance above the N20 motor");
...
assert(motor_clearance >= 0.5,
       "N20 pass-through needs at least 0.5 mm printed clearance");
...
assert(wheel_inner_face_x-cargo_shoulder_outer_x >= 2,
       "Cargo shoulder is too close to the purchased wheel inner face");
...
assert(housing_bottom_z-(motor_center_z+motor_height/2) >= 5,
       "Housing lower edge enters the N20 motor service envelope");
assert(housing_print_bottom_z-(motor_center_z+motor_height/2) >= 4,
       "Housing mounting bosses enter the N20 motor service envelope");
assert(wheel_inner_face_x-housing_outer_half_x >= 2,
...
assert(wheel_inner_face_x-housing_mount_boss_outer_x >= 2,
       "Housing mounting boss enters the purchased wheel envelope");
...
echo(str("VERIFY v0.7 complete envelope = ", robot_size));
...
echo(str("V0.7 N20 body = ",[motor_length,motor_width,motor_height],
         ", D shaft = ",motor_shaft_diameter," x ",motor_shaft_length));
echo(str("V0.7 battery nominal body = ",
         [battery_length,battery_width,battery_height],
         ", clearance per side = ",battery_clearance));
echo(str("V0.7 wheel centres = +/-",wheel_center_x,", Y",wheel_center_y,
         ", Z",wheel_center_z));
echo(str("V0.7 N20 pass-through clearance = ",motor_clearance));
echo(str("V0.7 shaft insertion into wheel hub = ",motor_shaft_insertion));
echo(str("V0.7 hardware overall width = ",hardware_overall_width));
echo(str("V0.7 battery/motor vertical clearance = ",
         motor_center_z-motor_height/2-battery_top_z));
echo(str("V0.7 chassis ground margin below body = ",
         chassis_ground_clearance-wheel_ground_clearance));
echo(str("V0.7 anti-tip support Y span = ",stability_skid_rear_y,
         " to ",stability_skid_front_y,
         "; reviewed structure CG Y = ",review_structural_cg_y));
echo(str("V0.7 CLOSED envelope = ",
         [hardware_overall_width,
          robot_max_y-min(gate_y-gate_barrel_d/2,-48),
          cargo_front_wall_top_z]));
echo(str("V0.7 housing main shell = ",
         [2*housing_outer_half_x,housing_front_y-housing_rear_y,
          housing_top_z-housing_bottom_z]));
echo(str("V0.7 housing including bosses = ",
         [2*housing_mount_boss_outer_x,
          housing_front_y-min(housing_rear_y,
                              housing_mount_yz[0][0]-housing_mount_boss_d/2),
          housing_top_z-housing_bottom_z]));
echo(str("V0.7 wheel axial clearance = ",
         wheel_inner_face_x-housing_outer_half_x));
echo(str("V0.7 servo roof clearance = ",
         housing_inner_top_z
...
module integrated_side_profile_2d() {
    difference() {
        hull() {
            // The purchased wheel is wholly outboard of this pod with 2 mm
            // axial clearance. The pod stays continuous around a small N20
            // pass-through instead of using a sill-severing wheel arch.
        ...
        translate([motor_center_y-motor_width/2-motor_clearance,
                   motor_center_z-motor_height/2-motor_clearance])
            square([motor_width+2*motor_clearance,
                    motor_height+2*motor_clearance]);
    }
}
...
module compact_front_carrier_plan_2d() {
    union() {
        ...
        for (sx=[-1,1]) {
            ...
            // Four local pads replace a full floor.  The removable battery
            // tray and motor cradles share these vertically stacked M3 joints.
            hull() {
                translate([sx*hamster_mount_x,hamster_mount_rear_y]) circle(r=4.5);
                translate([sx*battery_mount_x,battery_mount_y[0]])
                    circle(r=battery_mount_pad_d/2);
            }
            hull() {
                translate([sx*24,battery_mount_y[1]]) circle(r=3.2);
                translate([sx*battery_mount_x,battery_mount_y[1]])
                    circle(r=battery_mount_pad_d/2);
            }
        }
    }
...
module cargo_shoulder_right(yc) {
    ...
    hull() {
        ...
        translate([cargo_shoulder_outer_x-4,
                   yc-3,cargo_shoulder_rib_bottom_z]) cube([4,6,2]);
    }
}
...
module integrated_chassis_raw() {
    difference() {
        ...
        // Shared vertical M3 stack: chassis -> battery tray -> motor cradle.
        for (sx=[-1,1])
            for (yy=battery_mount_y) {
                translate([sx*battery_mount_x,yy,chassis_ground_z-0.1])
                    cylinder(d=battery_mount_hole_d,h=base_thickness+0.2);
                translate([sx*battery_mount_x,yy,chassis_ground_z-0.01])
                    cylinder(d1=6.6,d2=battery_mount_hole_d,h=1.7);
            }
...
// ---------------- v0.7 removable battery and drive mounts ----------------
module battery_tray_raw() {
    tray_outer_x = battery_length+2*battery_clearance;
    tray_outer_y = battery_width+2*battery_clearance;
    tray_side_x = tray_outer_x/2;
    difference() {
        union() {
            // Perimeter support keeps the pack low while removing central mass.
            translate([-tray_outer_x/2,
                       battery_center_y-tray_outer_y/2,
                       battery_tray_base_z])
                cube([tray_outer_x,tray_outer_y,battery_tray_thickness]);

            // Low end fences; a hook-and-loop strap provides vertical restraint.
            for (sx=[-1,1])
                translate([sx*(tray_side_x+0.9),battery_center_y,
                           battery_bottom_z+2.7])
                    cube([2.2,tray_outer_y,5.6],center=true);

            // Raised external tabs stack above the chassis pads. Horizontal
            // bridges start at the side fences, so no printed parts overlap.
            for (sx=[-1,1])
                for (yy=battery_mount_y) {
                    anchor_y = min(max(yy,
                        battery_center_y-tray_outer_y/2+1),
                        battery_center_y+tray_outer_y/2-1);
                    hull() {
                        translate([sx*(tray_side_x+1.2),anchor_y,
                                   battery_tray_tab_bottom_z])
                            cylinder(d=3,h=battery_tray_tab_thickness);
                        translate([sx*battery_mount_x,yy,
                                   battery_tray_tab_bottom_z])
                            cylinder(d=battery_mount_pad_d,
                                     h=battery_tray_tab_thickness);
                    }
                }
        }

        // Central lightening/access opening leaves a 6.8 mm support rim.
        translate([-(battery_length-12)/2,
                   battery_center_y-(battery_width-12)/2,
                   battery_tray_base_z-0.1])
            cube([battery_length-12,battery_width-12,
                  battery_tray_thickness+0.2]);

        // Two 14 x 3.2 mm slots accept a 10-12 mm hook-and-loop strap.
        for (yy=[battery_center_y-battery_width/2+4,
                 battery_center_y+battery_width/2-4])
            translate([-7,yy-1.6,battery_tray_base_z-0.1])
                cube([14,3.2,battery_tray_thickness+0.2]);

        for (sx=[-1,1])
            for (yy=battery_mount_y)
                translate([sx*battery_mount_x,yy,
                           battery_tray_tab_bottom_z-0.1])
                    cylinder(d=battery_mount_hole_d,
                             h=battery_tray_tab_thickness+0.2);
    }
}

module battery_tray_assembly(print_z0=false) {
    translate([0,0,print_z0 ? -battery_tray_base_z : 0])
        battery_tray_raw();
}

module drive_motor_mount_right_raw() {
    body_half_y = motor_width/2+motor_clearance;
    floor_z = motor_center_z-motor_height/2-motor_mount_wall;
    cradle_length = motor_mount_outer_x-motor_mount_inner_x;
    wall_h = motor_height+2*motor_clearance+motor_mount_wall;
    difference() {
        union() {
            translate([motor_mount_inner_x,
                       motor_center_y-body_half_y-motor_mount_wall,
                       floor_z])
                cube([cradle_length,
                      2*(body_half_y+motor_mount_wall),motor_mount_wall]);

            for (sy=[-1,1]) {
                wall_y = sy < 0
                         ? motor_center_y-body_half_y-motor_mount_wall
                         : motor_center_y+body_half_y;
                translate([motor_mount_inner_x,
                           wall_y,
                           floor_z])
                    cube([cradle_length,motor_mount_wall,wall_h],center=false);
            }

            // Two shared M3 feet and printable diagonal webs to the cradle.
            for (i=[0:1]) {
                yy = battery_mount_y[i];
                wall_y = i == 0
                         ? motor_center_y-body_half_y-motor_mount_wall
                         : motor_center_y+body_half_y;
                translate([battery_mount_x,yy,motor_mount_foot_bottom_z])
                    cylinder(d=battery_mount_pad_d,
                             h=motor_mount_foot_thickness);
                hull() {
                    translate([battery_mount_x-2.2,yy-2.2,
                               motor_mount_foot_bottom_z])
                        cube([4.4,4.4,motor_mount_foot_thickness]);
                    translate([motor_mount_inner_x,wall_y,floor_z])
                        cube([5,motor_mount_wall,motor_mount_wall]);
                }
            }
        }

        // Cable-tie passages provide positive vertical and axial retention.
        for (xx=[35,43])
            translate([xx-1.7,
                       motor_center_y-body_half_y-motor_mount_wall-0.1,
                       floor_z-0.1])
                cube([3.4,2*(body_half_y+motor_mount_wall)+0.2,
                      motor_mount_wall+0.2]);

        for (yy=battery_mount_y)
            translate([battery_mount_x,yy,motor_mount_foot_bottom_z-0.1])
                cylinder(d=battery_mount_hole_d,
                         h=motor_mount_foot_thickness+0.2);
    }
}

module drive_motor_mount_right(print_z0=false) {
    translate([0,0,print_z0 ? -motor_mount_foot_bottom_z : 0])
        drive_motor_mount_right_raw();
}

module drive_motor_mount_left(print_z0=false) {
    translate([0,0,print_z0 ? -motor_mount_foot_bottom_z : 0])
        mirror([1,0,0]) drive_motor_mount_right_raw();
}

module drive_mounts_assembly() {
...
module motor_reference_v07(alpha=0.42) {
    for (sx=[-1,1]) {
        color([0.72,0.72,0.74,alpha])
            translate([sx*motor_center_x,motor_center_y,motor_center_z])
                cube([motor_length,motor_width,motor_height],center=true);
        color([0.82,0.70,0.26,min(alpha+0.15,1)])
            translate([sx*(motor_outer_face_x+motor_shaft_length/2),
                       motor_center_y,motor_center_z])
                rotate([0,90,0])
                    cylinder(d=motor_shaft_diameter,
                             h=motor_shaft_length,center=true,$fn=24);
    }
}

module battery_reference_v07(alpha=0.40) {
    color([0.14,0.28,0.56,alpha])
        translate([0,battery_center_y,
                   battery_bottom_z+battery_height/2])
            cube([battery_length,battery_width,battery_height],center=true);
}

module hamster_core_reference_v04() {
    motor_reference_v07();
    battery_reference_v07();
}
...
} else if (part == "battery_tray") {
    battery_tray_assembly(true);
} else if (part == "motor_mount_right") {
    drive_motor_mount_right(true);
} else if (part == "motor_mount_left") {
    drive_motor_mount_left(true);
} else if (part == "wheel_reference") {
    cylinder(d=wheel_diameter,h=wheel_width);
} else if (part == "motor_reference") {
    translate([-motor_center_x,-motor_center_y,-(motor_center_z-motor_height/2)])
        motor_reference_v07(1);
} else if (part == "battery_reference") {
    translate([0,-battery_center_y,-battery_bottom_z])
